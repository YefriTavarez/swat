		...
		foreach ( $fields as $field ) {
			...
			if ( 'radio' === $fieldtype || 'checkbox' === $fieldtype ) {
				...
				foreach ( $field['choices'] as $choice_value => $choice_label ) {
					...
					$field_html .=  '<label class="choice choice-'. $choice_value .'"><input type="checkbox" name="'. $field['name'] .'[]" value="'. $choice_value .'" '. $checked .'> '. $choice_label. ' ('. $counter .')' .'</label><br>';
				}
				$field_html .= '</p>';
...
class pcd_Walker_CategoryDropdown extends Walker {
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= "\t<option data-make=\"$make->slug\" class=\"level-$depth\" value=\"".$category->slug."\"";
